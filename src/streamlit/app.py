        ...
        if images:
            ...
            for i, image in enumerate(images):
                with cols[i % 3]:
                    ...
                    if image_url:
                        try:
                            # Convert presigned URL to direct S3 URL
                            if '?' in image_url and 'AWSAccessKeyId' in image_url:
                                # Extract base URL before query parameters
                                direct_url = image_url.split('?')[0]
                                logger.info(f"Converted presigned URL to direct URL: {direct_url}")
                                image_url = direct_url
                            
                            # Use HTML img tag for direct browser loading (avoids Streamlit server-side fetch)
                            st.markdown(
                                f'<div style="text-align: center;">'
                                f'<img src="{image_url}" alt="{image.get("style", "Modern")} Style" '
                                f'style="max-width: 100%; height: auto; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">'
                                f'<p style="margin-top: 8px; color: #666; font-size: 14px;">{image.get("style", "Modern")} Style</p>'
                                f'</div>',
                                unsafe_allow_html=True
                            )
                        except Exception as e:
                ...
                with st.expander(f"{'✓ ' if is_selected else ''}Option {i+1}: {style.upper()} Style", expanded=(i == 0)):
                    ...
                    if image_url:
                        try:
                            ...
                            # Use HTML img tag for direct browser loading
                            st.markdown(
                                f'<div style="text-align: center;">'
                                f'<img src="{image_url}" alt="{style.upper()} Style Interior" '
                                f'style="max-width: 100%; height: auto; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">'
                                f'<p style="margin-top: 8px; color: #666; font-size: 14px;">{style.upper()} Style Interior</p>'
                                f'<p style="color: #888; font-size: 12px;">Generated by: {provider_label}</p>'
                                f'</div>',
                                unsafe_allow_html=True
                            )
                        except Exception as e:
