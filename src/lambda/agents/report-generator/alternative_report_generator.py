...
<body>
    ...
    def _generate_synthesized_insights_section_html(self, synthesized_insights: str) -> str:
        """Bedrock Claude로 종합된 인사이트 섹션 HTML 생성"""
        if not synthesized_insights:
            return ""
        
        # 인사이트를 단락으로 분리 (줄바꿈 기준)
        ...
        <div class="recommendations" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-left-color: #667eea;">
            <p style="font-weight: bold; margin-bottom: 15px; font-size: 16px;">🤖 Amazon Bedrock Claude가 분석한 종합 인사이트</p>
            {insights_html}
        </div>
        """
    ...
    def generate_text_report(self, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # AI 종합 인사이트 (Bedrock Claude)
            synthesized_insights = data.get('synthesized_insights', '')
            if synthesized_insights:
                report_lines.extend([
                    "✨ AI 종합 인사이트 (Amazon Bedrock Claude)",
                    "-" * 30
                ])
