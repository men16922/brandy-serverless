...
class S3Client:
    """AWS S3 client (AWS-only architecture)"""
    
    def __init__(self, environment: str = None):
        self.environment = 'dev'  # AWS-only
    ...
    def _get_bucket_name(self) -> str:
        ...
        # Default bucket name
        return 'ai-branding-chatbot-assets-908601828278'
    ...
    def _create_client(self):
        ...
        logger.info(f"Creating S3 client with IAM role in region: {region}")
    ...
    def _ensure_bucket_exists(self) -> None:
        ...
        try:
            ...
            if error_code == '404':
                ...
                # Forbidden - bucket exists but we don't have HeadBucket permission
                # This is OK - we can still upload files if we have PutObject permission
                logger.warning(f"No HeadBucket permission for {self.bucket_name}, but bucket likely exists. Continuing...")
            else:
