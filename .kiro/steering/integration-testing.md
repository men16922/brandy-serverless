...
**통합 테스트 우선 접근법**: 단위 테스트 대신 실제 환경과 유사한 Docker Compose 환경에서 end-to-end 통합 테스트만 수행합니다.
...
### DO (권장사항)
- 실제 서비스 연동으로 end-to-end 테스트 작성
- DynamoDB Admin UI를 활용한 데이터 시각적 검증
- Agent 간 통신 및 Supervisor 모니터링 실제 동작 테스트
- 오류 시나리오 및 폴백 메커니즘 검증
- 테스트 데이터 완전 정리 (다음 테스트 영향 방지)

### DON'T (지양사항)
- 단위 테스트 작성 (복잡성만 증가)
- Mock 객체 사용 (실제 연동 검증 불가)
